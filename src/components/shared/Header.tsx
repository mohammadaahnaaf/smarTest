        ...
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex md:items-center md:justify-end md:space-x-12">
            {navItems.map((x, index) => (
              <Link key={index} href="#" className="text-base font-normal text-gray-400 transition-all duration-200 hover:text-white"> {x.title} </Link>
            ))}
          </nav>
        </div>

        <nav className={expand ? `block` : `hidden`}>
